...
| D-020 | 2026-08-28 01:10 / Phase 7 | **Put OpenAI first in the provider chain, and prepare a bound fallback analysis for the demo conversation.** `.env` now sets `AI_PROVIDER=openai` / `AI_FALLBACK_PROVIDERS=anthropic,compatible`. Separately, `conflictSaturdayDinner.ts` holds a full prepared Conflict Lens analysis of the sixteen-message Saturday-dinner exchange, served automatically — and labelled on screen — when no provider answers. | (a) Leave Anthropic first and accept the wasted attempt. (b) Change the code default rather than `.env`. (c) On failure, fall back to any fixture. (d) No fallback: show the error. | The Anthropic account has no credit, so it returns 400 on every call and each request paid a failed attempt before failing over (measured: `failedOver: ["anthropic"]`, ~1.2s; after the swap, `failedOver: []`). That is an account condition, not a product decision, so it belongs in `.env` — the code default stays `anthropic`. On the fallback: option (c) is the D-009 violation, and (d) means the one conversation the room has just watched being uploaded produces an error. | The fallback is deliberately narrow. `matchesSaturdayDinner` requires **6 of 10 distinctive lines** — verified in the fixture gate to match its own conversation, to survive three messages being dropped as OCR may do, and **not** to match either the Alex/Sam fixture or a differently-worded argument about the same Saturday dinner. `buildSaturdayDinner` stamps `speakerId`s from the runtime speakers (a screenshot of this conversation contains no names, so the labels are only known after submission) and returns `null` rather than guessing when there are not exactly two speakers or none is marked as the user. The result renders behind an amber banner saying it is a saved read. | No. |
...
### Repair latency, measured

Measured in the running app on the sixteen-message Saturday-dinner conversation, OpenAI first:

| Event | Time |
|---|---|
| First read on screen | **2.1s** |
| Full analysis | **8.6s** |
| Bare relay round trip (`OK`, 10 tokens) | 1.2–1.5s |

Before the F-018 fix, neither ever appeared — both responses arrived and were discarded.
